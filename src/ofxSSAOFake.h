...
public:
        ...
        ofShader ssaoShader;
	bool fboActive,imgActive = false;

        ofxSSAOFake(){
            resolution.set(ofGetWidth(),ofGetHeight());
        ...
        void setOutputScene(ofImage img){
            ioutScene=img;
	    imgActive=true;
        }
        ...
        void draw(){
            ssaoShader.begin();
			if(fboActive==true)
				ssaoShader.setUniformTexture("tex0",foutScene.getTextureReference(),0);
			if(imgActive==true)
				ssaoShader.setUniformTexture("tex0",ioutScene.getTextureReference(),0);
